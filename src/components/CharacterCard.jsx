...
export const CharacterCard = ({ uid, name, dispatch, favorites }) => {
  const isFavorited = favorites.some(fav => fav.uid === uid && fav.category === "people");

  const toggleFavorite = () => {
    if (isFavorited) {
      dispatch({ type: "removeFavorite", payload: { uid, category: "people" } });
    } else {
      dispatch({ type: "favedCharacter", payload: { uid, name } });
    }
  };
